...
def _norm(s: str) -> str:
    """Нормализация для сверки цитат/span-ов: пробелы схлопываются, регистр
       снимается, markdown-выделение (**жирный**, *курсив*, `код`, ~зачёркнутое~)
       срезается — иначе дословная цитата «Ротация ghp_-токена» не находится во
       фрагменте «**Ротация ghp_-токена**» и честный голос судьи пропадает.
       Буквы не трогаем: «Настроек» ≠ «Настройки» по-прежнему проваливается."""
    s = re.sub(r"[*_`~]", "", s)
